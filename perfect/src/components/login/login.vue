<template>
  <div>
    <div id="loginWrapper">
      <div class="old_user_login"><a href="#/usercenter" class=""><label class="acct_top_back"></label></a> <img
        src="data:image/png;base64,"
        class="acct_top_show"></div>
      <div class="bg-wrap" style="height: 526px;">
        <div class="login_content"><h1 data-v-4e187200="" style="padding-bottom: 0px;">老用户登录</h1>
          <div id="wrap">
            <form id="register-form ">
              <ul class="fm_list register_distance " id="loginlist">
                <li><input id="txtMobile" type="tel" maxlength="11" placeholder="请输入手机号" required="required"
                           class="field_ipt"></li>
                <li><input type="text" placeholder="请输入登录密码" maxlength="10" id="regSmsCaptcha" required="required"
                           class="field_ipt"> <a class="fidld_skip">忘记密码</a></li>
                <div id="setting">
                  <router-link to="javascript:;">
                    <span data-v-4e187200="">手机号快捷登录</span>
                    <b data-v-4e187200=""></b>
                  </router-link>
                </div>
                </ul>
             <input data-v-4e187200="" type="button" value="注册"
                                                           class="btn_login space m-top"> <a href="#/login"
                                                                                             class=""><input
              type="button" value="登录" class="btn_regisiter space m-top"></a></form>
          </div>
        </div>
      </div>
    </div>

  </div>
</template>

<script>
  export default {}
</script>

<style>
 #setting {
    text-align: right;
    height: 50px;
    width: 90%;
    margin: 0 auto;
    font-size: 16px;
    box-sizing: border-box;
    position: relative;
   color:#3aad36;
  }
 #loginlist{
   margin-bottom: 0rem;
   margin-top: 3rem;
 }
 #setting a {
   float: right;
   line-height: 20px;
   margin-right: 7px;
   margin-top: 10px;
   color: #3aad36;
 }
 #setting a b{
   width: 8px;
   height: 8px;
   display: block;
   position: absolute;
   right: -.5rem;
   top: 1.48rem;
   border-top: 1px solid;
   border-right: 1px solid;
   transform: rotate(45deg);
   -ms-transform: rotate(45deg);
   -moz-transform: rotate(45deg);
   -webkit-transform: rotate(45deg);
 }

</style>
